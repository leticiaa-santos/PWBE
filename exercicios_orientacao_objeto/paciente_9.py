...
class Paciente:
    ...
    def nova_consulta(self, consulta):
        self.historico.append(consulta) # adiciona uma nova consulta

    def mostrar_historico(self):
        if not self.historico: # verifica se o paciente tem alguma consulta
            print(f"O paciente {self.nome} ainda não tem nenhuma consulta")
        else:
            print(f"O paciente {self.nome} possui as consultas: ") 
            for i, consulta in enumerate(self.historico, 1): # lista as consultas do paciente
                print(f"{i} - {consulta}")
