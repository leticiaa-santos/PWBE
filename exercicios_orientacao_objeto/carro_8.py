...
class Carro:
    ...
    def acelerar_carro(self):
        self.acelerar = self.velocidade_atual + self.mudar_velocidade # adiciona a velocidade informada na velocidade anterior
    ...
    def frear_carro(self):
        if self.velocidade_atual <= 0: # verifica se o carro já está parado para frear
            print("O carro já está parado, não é possível frear")
        else:
            self.frear = self.velocidade_atual - self.mudar_velocidade # diminui a velocidade informada na velocidade anterior
