...
class Livro:
    ...
    def emprestar_livro(self):
        if self.livro_disponivel:
            self.livro_disponivel = False # muda a disponibilidade do livro que foi emprestado para False
    ...
    def devolver_livro(self):
        if not self.livro_disponivel:
            self.livro_disponivel = True # muda a disponibilidade do livro que foi devolvido para True
            print(f"Você devolveu o livro {self.titulo} com sucesso")
        else:
            print("O livro já foi devolvido, essa operação não pode ser realizada")

    def verificar_disponibilidade(self): # mostra a disponibilidade do livro
        if self.livro_disponivel:
