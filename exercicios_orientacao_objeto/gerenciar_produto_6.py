...
class Produto:
    ...
    def valor_estoque(self):
        self.valor_total = self.preco * self.quantidade # calculando o valor total que está no estoque de acordo com o preco e a quatidade que tem
        print(f"No estoque tem {self.quantidade} {self.nome} totalizando R$ {self.valor_total:,.2f}")

    def disponivel(self):
        if self.quantidade > 0: # varificando a disponibilidade do produto
